...
function modifyContractLinesXml(sheetXml, fixedData, fixedCells, greenXfIdx){
  // Text-based approach: avoids XMLSerializer namespace re-declaration issues that
  // caused newly created cells to show green highlight but no value in Excel.
  let xml = sheetXml;
  const enc = s => String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');

  // Build replacement cell XML string
  ...
  // Find position of '>' that closes a row opening tag e.g. <row r="3" spans="...">
  ...
  // Find and replace an existing cell element, return new xml or null if not found
  const replaceCell = (xmlStr, ref, newCellXml) => {
    const rAttr = `r="${ref}"`;
    let rPos = xmlStr.indexOf(rAttr);
    while(rPos !== -1){
      // Verify the character after closing quote is a valid attribute separator or tag end
      const ch = xmlStr[rPos + rAttr.length];
      if(ch === ' ' || ch === '>' || ch === '/' || ch === '\n' || ch === '\t'){
        // Walk back to find opening '<' of the cell element
        let start = rPos - 1;
        while(start > 0 && xmlStr[start] !== '<') start--;
        if(xmlStr.slice(start, start + 2) === '<c'){
          // Find end of cell element (self-closing or with </c>)
          let tagEnd = rPos;
          while(tagEnd < xmlStr.length && xmlStr[tagEnd] !== '>') tagEnd++;
          let cellEnd;
          if(xmlStr[tagEnd - 1] === '/'){
            cellEnd = tagEnd + 1; // self-closing
          } else {
  ...
  for(const [rowNum, cols] of fixedCells){
    const rowData = fixedData[String(rowNum)] || fixedData[rowNum] || {};
    for(const col of cols){
      const ref = col + rowNum;
      const val = rowData[col] ?? '';
      const newCell = buildCell(ref, val, greenXfIdx);

      // Try to replace existing cell first
      const replaced = replaceCell(xml, ref, newCell);
      if(replaced !== null){
        xml = replaced;
      } else {
        // Cell doesn't exist in original XML — insert right after row opening tag
        const rowEnd = findRowTagEnd(xml, rowNum);
        if(rowEnd !== -1){
          xml = xml.slice(0, rowEnd + 1) + newCell + xml.slice(rowEnd + 1);
        }
      }
    }
  }
