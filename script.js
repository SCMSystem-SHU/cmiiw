...
function runValidation(data, lastRow){
  const logs = [];
  const add = (errorId, kolom, type, msg, line) => logs.push({ errorId, kolom, line, type, msg, linefix: FIXABLE_IDS.has(errorId) ? 'V' : '' });
  ...
  for(let i = 2; i <= lastRow; i++){
    ...
    // Point.9a - AH tidak boleh ada tanda baca titik
...
function setFile(file){
  ...
  closeFilterPanel();
  activeFilters = { errorId: null, kolom: null, line: null, type: null, msg: null, linefix: null };
  ...
  badgeWarning.textContent = '';
  badgeLineFix.textContent = '';
...
let currentLogs = [];
let activeFilters = { errorId: null, kolom: null, line: null, type: null, msg: null, linefix: null };
...
function renderTable(){
  ...
  if(rows.length === 0){
    resultBody.innerHTML = '<tr><td colspan="6" class="empty-state">Tidak ada baris yang cocok dengan filter saat ini.</td></tr>';
    return;
  }
  for(const l of rows){
    ...
    const linefixVal = FIXABLE_IDS.has(l.errorId) ? 'V' : '';
    tr.innerHTML =
      ...
      '<td class="col-linefix">' + (linefixVal ? '<strong>V</strong>' : '') + '</td>';
...
function renderResults(logs){
  ...
  // Count total LOG ROWS with fixable errorId (= total checkmarks in "Line Fixing" column)
  const fixingLineCount = logs.filter(l => FIXABLE_IDS.has(l.errorId)).length;
...
function buildCheckingLogSheet(logs, fixedLogRows, xfError, xfWarn, xfHeader){
  ...
  // Headers: NO Error ID (point 3), ADD Line Fixing (point 2)
  const newHeaders=['Kolom','Line','Type','Logs','Line Fixing'];
  let properRows='<row r="1">';
  newHeaders.forEach((h,ci)=>{
  ...
  logs.forEach((l,idx)=>{
    ...
    const linefixVal=FIXABLE_IDS.has(l.errorId)?'V':'';
    properRows+=`<row r="${rn}">`;
    [String(l.kolom),String(l.line),String(l.type),String(l.msg),linefixVal].forEach((v,ci)=>{
      const sA=xf>0?` s="${xf}"`:'';
      properRows+=`<c r="${colNumToLetter(ci+1)+rn}" t="inlineStr"${sA}><is><t>${enc(v)}</t></is></c>`;
    });
    properRows+='</row>';
  });
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><cols><col min="1" max="1" width="12" customWidth="1"/><col min="2" max="2" width="8" customWidth="1"/><col min="3" max="3" width="10" customWidth="1"/><col min="4" max="4" width="80" customWidth="1"/><col min="5" max="5" width="14" customWidth="1"/></cols><sheetData>${properRows}</sheetData></worksheet>`;
}
